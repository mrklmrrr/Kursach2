...
import { adjustHeaders, adjustColUnits, resizeTemplateCells } from '@utils/gridUtils';
...
export default function GridTemplateBuilder({
  ...
  return (
    <div className="research-form lab-template-builder">
      <div className="lab-template-builder-top">
        <div>
          <h3 className="lab-template-builder-title">
            {editingTemplateId ? 'Редактирование шаблона' : (mode === 'grid' ? 'Новый бланк (шаблон таблицы)' : 'Новый шаблон')}
          </h3>
          <p className="lab-help lab-template-builder-sub">
            {mode === 'grid'
              ? 'Укажите название анализа — по нему вы и пациент будете находить записи. Нажмите «Сохранить бланк», чтобы использовать таблицу как общепринятый перечень показателей.'
              : 'Укажите название анализа и сохраните шаблон.'}
          </p>
        </div>
      </div>
      ...
      {onTemplateModeChange && (
        <div className="lab-template-mode-toggle">
          <label className="checkbox-label">
            <input
              type="checkbox"
              checked={mode === 'grid'}
              onChange={(e) => onTemplateModeChange(e.target.checked ? 'grid' : 'fields')}
            />
            <span>Таблица</span>
          </label>
        </div>
